...
export default function ChannelsExplorer({ rows, initialSort }: Props) {
  ...
  const [appliedFilters, setAppliedFilters] = useState([province, status]);
  ...
  if (appliedFilters[0] !== province || appliedFilters[1] !== status) {
    setAppliedFilters([province, status]);
    setPage(1);
  }
  ...
  const filtered = useMemo(
    () =>
      rows.filter(
        (row) => (!province || row.province === province) && (!status || row.status === status),
      ),
    [rows, province, status],
  );
  ...
  const sorted = useMemo(() => {
    const copy = [...filtered];
    if (initialSort === 'frequency') {
      copy.sort((a, b) => {
        if (a.frequencyMhz === null && b.frequencyMhz === null) return 0;
        if (a.frequencyMhz === null) return 1;
        if (b.frequencyMhz === null) return -1;
    ...
    return copy;
  }, [filtered, initialSort]);

  const hasActiveFilters = province !== '' || status !== '';
        ...
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-text-secondary text-sm">
              ...
              Filtrá por provincia o estado para acotar la búsqueda.
            </p>
          </div>
        </div>
          ...
          <table className="w-full min-w-[820px] text-sm">
            <thead className="bg-bg-secondary text-text-secondary text-left text-xs font-medium tracking-wide uppercase">
              <tr>
                ...
                <th className="px-4 py-3">Canal virtual</th>
                <th className="px-4 py-3">Canal físico</th>
